...
public final class IjkMediaPlayer extends AbstractMediaPlayer {
    ...
    /**
     * jni 全局初始化只调用一次或者成对出现 设置缓存路径和最大缓存时长
     *
     * @param cachePath    TS 缓存目录
     * @param maxDurationS 最大缓存时长 s,360s-600s之间
     */
    public static native void native_tsInitCacheManager(String cachePath, long maxDurationS);

    /**
     * jni 退出播放器只调用一次或者成对出现
     */
    public static native void native_tsDestoryHttpCache();

    /**
     * 外部方法
     * 根据当前时间点获取ts文件路径，如果ts文件没有下载，将下载完毕后再返回,如果超时返回已经有的的ts文件
     *
     * @param begin_time_ms
     * @param end_time_ms
     * @param time_out_s       超时时间
     * @param ts_files         out 文件名称
     * @param ts_start_time_ms out
     * @param ts_duration_ms   out
     * @return
     */
    public static native  String[] native_tsGetOrDownloadTsFiles(long begin_time_ms, long end_time_ms,
                                                           long time_out_s, String[] ts_files,
                                                           long ts_start_time_ms, long ts_duration_ms);

    /**
     * 外部方法
     * get合成mp4后必须调用该方法，释放ts_file锁和内存
     *
     * @param ts_file
     */
    public static native void native_TsGetTsFinished(String[] ts_files, int size);
